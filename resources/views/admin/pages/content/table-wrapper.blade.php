...
<table id="kt_datatable_basic" class="table align-middle table-row-dashed fs-6 gy-5">
    ...
    <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
        ...
        <th>#</th>
        <th>{{__('admin.ps_page_name')}}</th>
        <th>{{__('admin.ps_page_title')}}</th>
        <th>{{__('admin.ps_page_slug')}}</th>
        <th>{{__('admin.ps_page_status')}}</th>
        <th>{{__('admin.sh_date')}}</th>
        <th class="text-center">{{__('admin.ps_page_options')}}</th>
    </tr>
    </thead>
    <tbody class="text-gray-600 fw-semibold" id="table-wrapper">
        @include('admin.pages.content.show')
    </tbody>
</table>
{{ $pages->links() }}
<!--end::Datatable-->
